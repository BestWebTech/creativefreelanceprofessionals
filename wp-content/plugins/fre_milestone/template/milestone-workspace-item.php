...
<li class="item-list-milestone">
	<div class="item-list-mileston-wrapper <?php echo $milestone->class_label; ?>">
		...
		<div class="content-steps-milestone">
			...
			<div class="txt-status-milestone">
				<a class="change-status-milestone" href="javascript:void(0);">
					...
					<?php
					if ( $current_user->ID == $author_id && $milestone->post_status != 'done' ) {
						echo '<i class="fa fa-angle-down"></i>';
					} elseif( $current_user->ID != $author_id && $milestone->post_status != 'resolve' && $milestone->post_status != 'done' ) {
						echo '<i class="fa fa-angle-down"></i>';
					}
					?>
				</a>

				<?php if( $current_user->ID == $author_id && $milestone->post_status != 'done' ) { ?>
				...
				<?php } elseif( $current_user->ID != $author_id && $milestone->post_status != 'resolve' && $milestone->post_status != 'done' ) { ?>
				<ul class="cat-action-milestone">
