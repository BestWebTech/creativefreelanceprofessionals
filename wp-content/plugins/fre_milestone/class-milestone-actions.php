...
class AE_Milestone_Actions extends AE_Base
{
	 ...
	 * @author tatthien
	 */
	public function ae_after_insert_milestone( $result, $args ) {
	...
	public function ae_update_milestone_after_edit_project( $result, $args ) {
		if( isset( $args['milestones_id'] ) && !empty( $args['milestones_id'] ) && isset( $args['milestones'] ) && !empty( $args['milestones'] ) ) {
			...
			if( $project->post_status != 'publish' && $project->post_status != 'pending' ) {
				wp_send_json( array(
					'success' => false,
					'msg' => __( "Can not update milestone when project in process" )
				) );
			}
	...
	public function ae_milestone_sync() {
		...
		if(  $user_ID == $bid_author && $request['do_action'] == 'resolve_milestone' ) {
			$result = $this->ae_update_milestone( $request );
		} elseif( $user_ID == $project_author ) {
			$milestone = $ae_post_factory->get( 'ae_milestone' );
			...
			if( $project_status != 'complete' && $project_status != 'disputing' ) {
				...
				wp_send_json( array(
					'success' => false,
					'msg' => __( "Project not in process" ,ET_DOMAIN )
				) );
			}
		...
		if( !is_wp_error( $result ) ) {
			switch ( $request['do_action'] ) {
				case 'resolve_milestone':
					...
					$msg = __( 'You has resolved a milestone.', ET_DOMAIN );
				...
				case 'close_milestone':
					...
					$msg = __( 'You has closed a milestone.', ET_DOMAIN );
				...
				case 'reopen_milestone':
					...
					$msg = __( 'You has re-opended milestone.', ET_DOMAIN );
	 ...
	 * @param string $action 	Milesteon action (reopen, resolve, close, remove...)
	 * @return void
	 *
	 * @since 1.0
	 * @package FREELANCEENGINE
	 ...
	 * @return object $result 			The return of changelog data
	 *
	 * @since 1.0
	 * @package FREELANCEENGINE
	 * @category MILESTONE
	...
	public function ae_add_milestone_meta( $result ) {
		switch ( $result->post_status ) {
			...
			case 'reopen':
				$result->status_label = __( 'Re-open', ET_DOMAIN );
				$result->class_label = 'opened';
				break;

			case 'resolve':
				$result->status_label = __( 'Resolve', ET_DOMAIN );
				$result->class_label = 'resolved';
				break;

			case 'done':
				$result->status_label = __( 'Close', ET_DOMAIN );
	...
	public function ae_add_notification_for_milestone_action( $action, $milestone, $project_id, $user_id ) {
		...
		switch ( $action ) {
			case 'reopen':
				...
				$post_title = sprintf( __( 'Milestone %s on project %s was re-opened', ET_DOMAIN ), $milestone->post_title, $project->post_title );
				break;

			case 'resolve':
				$notify_content = sprintf( 'type=resolve_milestone&milestone=%s&project=%s', $milestone->ID, $project_id );
				$post_title = sprintf( __( 'Milestone %s on project %s was resolved', ET_DOMAIN ), $milestone->post_title, $project->post_title );
				break;

			case 'close':
				$notify_content = sprintf( 'type=close_milestone&milestone=%s&project=%s', $milestone->ID, $project_id );
				$post_title = sprintf( __( 'Milestone %s on project %s was closed', ET_DOMAIN ), $milestone->post_title, $project->post_title );
				break;

			case 'create':
				$notify_content = sprintf( 'type=create_milestone&milestone=%s&project=%s', $milestone->ID, $project_id );
				$post_title = sprintf( __( 'Milestone %s on project %s was created', ET_DOMAIN ), $milestone->post_title, $project->post_title );
				break;
		}
