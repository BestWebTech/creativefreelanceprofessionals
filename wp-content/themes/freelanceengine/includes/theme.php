...
if( ! function_exists( 'fre_show_credit' ) ):
    ...
    function fre_show_credit($user_role) {
        ...
        * only show credit number if current user is freelancer or share role and employer role
         */
        if( ae_get_option('pay_to_bid', false) && ( $user_role == FREELANCER || ( fre_share_role() && in_array( $user_role, array(FREELANCER, EMPLOYER) ) ) ) ) {

            $credits_a_bid  = (int) ae_get_option( 'ae_credit_number', 0 );
            $credits        = get_user_credit_number( $user_ID );
            $free_bid       = (int) ae_get_option( 'ae_free_bid_number', 0 );
            $free_bided     = (int) get_user_meta($user_ID, 'ae_user_free_bid', true);
            $free_bid_left  = max( intval($free_bid - $free_bided), 0);
            $bid_left       = $free_bid_left + floor( $credits/$credits_a_bid );
            ?>
            ...
            <div class="form-group form-group-mobile">
                <div class="form-group-control">
                    ...
                    <?php
                    if( $bid_left == 1 ){
                        printf( __ ('You are left with %1$s bid(%2$s credits), recharge it to bid more.', ET_DOMAIN), $bid_left, $credits );
                    } else {
                        printf( __ ('You are left with %1$s bids(%2$s credits), recharge it to bid more.', ET_DOMAIN), $bid_left, $credits );
                    }
                    ?>
