...
function fre_new_ae_global($vars){
    ...
    //$free_bid = get_user_meta($user_ID, 'ae_user_free_bid');
    //$free_bid = empty($free_bid) ? '0': $free_bid;

    $free_bid       = (int) get_user_meta( $user_ID, 'ae_user_free_bid', true );
    $op_free_bid    = (int) ae_get_option( 'ae_free_bid_number', 0 );
    $bid_dis        = (int) ae_get_option( 'ae_credit_number', 0 );
    $credits        = get_user_credit_number( $user_ID );
    $free_bid_left  = (int)$op_free_bid - (int)( $free_bid + 1 );

    if( $free_bid_left > 0 ){
        if($free_bid_left > 1){
            $msg = sprintf(__('You have bid successfully. Only %s free bids left', ET_DOMAIN), $free_bid_left);
        }else{
            $msg = sprintf(__('You have bid successfully. Only %s free bid left', ET_DOMAIN), $free_bid_left);
        }
    }else{
        $credits_left = $credits - $bid_dis;
        if( $credits_left > 0 ){
            if($credits_left > 1){
                $msg = sprintf( __('You have bid successfully. Only %s credits left', ET_DOMAIN), $credits_left );
            }else{
                $msg = sprintf( __('You have bid successfully. Only %s credit left', ET_DOMAIN), $credits_left );
            }
        }else{
            $msg = sprintf( __('You have bid successfully. No credit left', ET_DOMAIN), $credits_left );
        }
    }

    $vars['bid_success_msg'] = $msg;
