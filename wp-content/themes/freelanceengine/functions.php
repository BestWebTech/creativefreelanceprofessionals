...
class ET_FreelanceEngine extends AE_Base
{
    function __construct() {
        ...
        /**
         * add new role Admin Staff
         */
        if (!isset($wp_roles->roles[ADMINSTAFF])) {
            add_role(ADMINSTAFF, __('Admin Staff', ET_DOMAIN) , array(
                'read' => true,
...
//add_action( 'fre_profile_tabs', 'add_profile_info' );
//function add_profile_info($other_profile_field){
//    $other_profile_field  ?>
  <!--  <li>
//    <a href="#tab_education_details" role="tab" data-toggle="tab">
//        <?php// _e('Personal Information and education info', ET_DOMAIN); ?>
//      </li> --> <?php;
//    echo  $other_profile_field;
//}


add_filter( 'ae_define_user_meta', 'add_profile_info',10);
function add_profile_info($meta_data){
//pr($meta_data);die;
//pr($meta_data);   
...
$meta_data = array_merge($meta_data_array, $meta_data); 
$login_userid= get_current_user_id();
$website = 'employer';
...
add_action('wp_logout', 'myplugin_registration_save', 10,1);
function myplugin_registration_save( $userId ) {
      $userId= get_current_user_id();
     $currentUserRole = getUserRole($userId);
     $registeredUserRole = get_user_meta($userId, 'registered_user_role', true);
     if($currentUserRole != $registeredUserRole) {
         $newUserRole= new WP_User($userId);
        $newUserRole->set_role($registeredUserRole); 
       
        //update_user_meta($user_id, 'capabilities', $registeredUserRole);
     }
}

add_action( 'user_register', 'add_usermeta_role', 10, 1 );
function add_usermeta_role( $user_id ) {
    global $userCapabilities;
    $userRole = getUserRole($user_id);
  update_user_meta($user_id, 'registered_user_role', $userRole);
  
}

add_action( 'init', 'switch_user_role', 10);
function switch_user_role() {
   global $current_user;
     if(isset($_GET['switch_user']) && ($_GET['switch_user'] == 'switch-user')) {
        get_currentuserinfo();
        $userId =  $current_user->ID;
        $userRole = getUserRole($userId);
        $userRole = $userRole == 'freelancer' ? 'employer' : 'freelancer';
        $newUserRole= new WP_User($userId);
        $newUserRole->set_role($userRole); 
    }
}
...
function roleCheckBox($checkBox){
    $currnentUserId= get_current_user_id();
   global $current_user, $wpdb;
    ...
    if($role == $employer){
        echo "<a value='freelancer' class='freelancer' href='?switch_user=switch-user'>
         <i class='fa fa-plus-circle'></i>Switch user to freelancer</a>"; 
    }
   
    else if($role == $freelancer){
         echo "<a value='employer' class='employer' href='?switch_user=switch-user'>
         <i class='fa fa-plus-circle'></i>Switch user to employer</a>"; 
    }
}
add_shortcode('roleBox','roleCheckBox');
...
function my_myme_types($mime_types){
    ...
    $mime_types['vid8'] =  '.gvi'; 
    $mime_types['vid9'] =  '.wmv'; 
    $mime_types['vid10'] = '.pdf'; 
    $mime_types['vid11'] = '.docx'; 
    $mime_types['vid12'] = '.txt';  
...
function my_user_vote() {
    ...
    $new_vote_count = $_REQUEST["post_id"];
    $vote_count = get_user_meta($currentUserId, "_save_postids", true);
    $vote_count = ! empty($vote_count) ? unserialize($vote_count) : '';
    
    
    if(! empty($vote_count)) {
        $savePost = array_merge($vote_count ,array($new_vote_count));    
    } else {
        $savePost = array($new_vote_count);
    }
    
   
    $savePostValue = serialize ($savePost ); 
    $vote = update_user_meta($currentUserId, "_save_postids", $savePostValue);
    $result['type'] = 'success';
      $result['message'] = 'Project has been successfully saved.';
...
function my_script_enqueuer() {
   wp_register_script( "my_voter_script", WP_PLUGIN_URL.'/custom-saved-post/custom.js', array('jquery') );
